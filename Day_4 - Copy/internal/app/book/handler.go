...
func (h *handler) Get(c echo.Context) error {
	...
	res, err := h.service.Get(c.Request().Context(), &dto.BookRequest{ID: uint(id)})
	if err != nil {
		return response.ErrorResponse(err).Send(c)
	}
...
func (h *handler) Find(c echo.Context) error {
	...
	return response.CustomSuccessBuilder(response.SuccessConstant.OK.Code, res.Data, "get books success", &res.PaginationInfo).Send(c)
}
...
func (h *handler) Delete(c echo.Context) error {
	...
	err := h.service.Delete(c.Request().Context(), &dto.BookRequest{ID: uint(id)})
	if err != nil {
		return response.ErrorResponse(err).Send(c)
	}

	return response.SuccessResponse("success delete book").Send(c)
}
